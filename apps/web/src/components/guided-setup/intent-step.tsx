...
export function IntentStep({
  ...
  const [saveError, setSaveError] = useState<GuidedSetupErrorMessageKey | null>(null);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSaveError(guidedSetupErrorMessageKey(error));
    } finally {
      setSubmitting(false);
    }
  }
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        <fieldset
          ...
          aria-describedby={requiredError ? "guided-setup-intent-error" : undefined}
          className="space-y-3"
        >
          <legend
        ...
        {requiredError && (
          <p id="guided-setup-intent-error" role="alert" className="text-sm text-red-600">
