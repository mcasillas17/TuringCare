      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <input
                ...
                onKeyDown={(event) => {
                  ...
                  if (event.key === "ArrowDown" || event.key === "ArrowRight") {
                    event.preventDefault();
                    setIntent(intents[(index + 1) % intents.length]?.value ?? option.value);
                    return;
                  }
                  if (event.key === "ArrowUp" || event.key === "ArrowLeft") {
                    event.preventDefault();
                    setIntent(
                      intents[(index - 1 + intents.length) % intents.length]?.value ?? option.value,
                    );
                  }
                }}
        ...
        {submitError && (
          <p role="alert" className="text-sm text-red-600">
            {intent ? t("guidedSetup.intentError") : t("guidedSetup.selectionRequired")}
          </p>
        )}
