  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
          ...
          <li key={e.id} className="rounded border border-silver bg-white p-3 text-sm">
            <div className="flex justify-between">
              <span className="text-slate-soft">
                {String(e.occurredAt).slice(0, 16).replace("T", " ")} · {t("journal.intensity")}:{" "}
                {e.intensity}
              </span>
              <Button variant="outline" onClick={() => del.mutate(e.id)}>
                {t("journal.remove")}
              </Button>
            </div>
            <div>A: {e.antecedent}</div>
            <div>B: {e.behavior}</div>
            <div>C: {e.consequence}</div>
      ...
      <form onSubmit={onSubmit} className="space-y-3 rounded border border-silver bg-white p-4">
        ...
        <label className="block">
          <span className="text-sm">{t("journal.location")}</span>
          <input
            className={input}
        ...
        <label className="block">
          <span className="text-sm">{t("journal.notes")}</span>
          <textarea
            className={input}
            rows={2}
