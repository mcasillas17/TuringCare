...
- Change database structure in `apps/api/src/db/schema.ts`, generate a committed migration with `pnpm db:generate`, and use `db:migrate` for committed migrations/CI. `db:push` is for applying the current schema during local setup. The current production tail has a fail-closed split after pre-deploy migration 0022 and before post-deploy migrations 0023/0024; any later migration must choose and test a rollout phase and update `migrate-predeploy.ts` plus workflow contract tests.
